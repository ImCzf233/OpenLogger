...
public final class GLUProjection
{
        ...
        if (!outsideFrustum) {
            ...
            Label_0967: {
                ...
                if ((extrudeInverted && !outsideInvertedFrustum) || clampModeOutside == ClampMode.DIRECT) {
                    ...
                    double guiX3 = this.coords.get(0) * this.widthScale;
                    double guiY3 = (this.displayHeight - this.coords.get(1)) * this.heightScale;
                    if (opposite) {
                        guiX3 = this.displayWidth * this.widthScale - guiX3;
                        guiY3 = this.displayHeight * this.heightScale - guiY3;
                    }
                    if (guiX3 < 0.0) {
                        guiX3 = 0.0;
                    }
                    else if (guiX3 > this.displayWidth * this.widthScale) {
                        guiX3 = this.displayWidth * this.widthScale;
                    }
                    if (guiY3 < 0.0) {
                        guiY3 = 0.0;
                        return new Projection(guiX3, guiY3, Projection.Type.OUTSIDE);
                    }
                    if (guiY3 <= this.displayHeight * this.heightScale) {
                        return new Projection(guiX3, guiY3, Projection.Type.OUTSIDE);
                    }
                    guiY3 = this.displayHeight * this.heightScale;
                    return new Projection(guiX3, guiY3, Projection.Type.OUTSIDE);
                }
            }
            if (!GLU.gluProject((float)x, (float)y, (float)z, this.modelview, this.projection, this.viewport, this.coords)) {
            ...
            double guiY3 = (this.displayHeight - this.coords.get(1)) * this.heightScale;
            if (!opposite) {
                return new Projection(guiX3, guiY3, outsideInvertedFrustum ? Projection.Type.OUTSIDE : Projection.Type.INVERTED);
            }
            guiX3 = this.displayWidth * this.widthScale - guiX3;
            guiY3 = this.displayHeight * this.heightScale - guiY3;
            return new Projection(guiX3, guiY3, outsideInvertedFrustum ? Projection.Type.OUTSIDE : Projection.Type.INVERTED);
    ...
    public static class Line
    {
        ...
        private Vector3D getResult(final double te, final double be) {
