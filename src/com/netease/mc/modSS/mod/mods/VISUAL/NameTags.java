...
public class NameTags extends Mod
{
    ...
    public NameTags() {
        super("NameTags", "Allows you to see armor of player or info of item", Category.VISUAL);
        this.addSetting(this.armor = new Setting("Armor", this, true));
    }
    ...
    public void onRender3D(final Event3D event) {
        for (final Object object : Utils.getEntityList()) {
            if (object instanceof EntityLivingBase) {
                ...
                final double xPos = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * event.partialTicks - renderPosX;
                final double yPos = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * event.partialTicks - renderPosY;
                final double zPos = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * event.partialTicks - renderPosZ;
    ...
    void renderNameTag(final EntityLivingBase entity, String tag, final double x, double y, final double z) {
        ...
        int color = Colors.color(1.0f, 1.0f, 1.0f, 0.5f);
        final EntityPlayerSP player = Wrapper.INSTANCE.player();
        final CFontRenderer fontRenderer = FontManager.verdana_bold;
        ...
        if (entity instanceof EntityPlayer) {
            ...
            if (ValidUtils.isBot((EntityLivingBase)entityPlayer)) {
                tag = "¡ìe" + ID;
                color = Colors.color(200.0f, 200.0f, 0.0f, 160.0f);
            }
        }
        final int health = (int)entity.getHealth();
        tag = "" + Math.round(health);
        final RenderManager renderManager = Wrapper.INSTANCE.mc().getRenderManager();
        ...
        if (entity instanceof EntityPlayer && this.armor.isEnabled()) {
            ...
            this.renderArmor(entityPlayer2, 0, -(fontRenderer.getHeight() + 1) - 20);
            GlStateManager.translate(0.0f, -1.0f, 0.0f);
        }
        GL11.glPushMatrix();
        GL11.glPopMatrix();
    ...
    public void renderArmor(final EntityPlayer player, int x, final int y) {
        ...
        final ItemStack inHand = player.getHeldItem();
